...
void Mass_Storage_In (void)
{
  switch (Bot_State)
  {
    ...
    case BOT_ERROR:
      Bot_State = BOT_IDLE;
      SetEPRxStatus(ENDP3, EP_RX_VALID);/* enable the Endpoint to receive the next cmd*/
      if (GetEPRxStatus(EP3_OUT) == EP_RX_STALL)
      {
        SetEPRxStatus(EP3_OUT, EP_RX_VALID);/* enable the Endpoint to receive the next cmd*/
      }
      break;
    case BOT_DATA_IN:
      switch (CBW.CB[0])
      {
        case SCSI_READ10:
					MSD_LAST_ACCESS = SystickCount_ms ;
...
{
  USB_SIL_Write(EP3_IN, Data_Pointer, Data_Len);
...
void Set_CSW (uint8_t CSW_Status, uint8_t Send_Permission)
{
  ...
  USB_SIL_Write(EP3_IN, ((uint8_t *)& CSW), CSW_DATA_LENGTH);
