...
class canSnifferGUI(QMainWindow, canSniffer_ui.Ui_MainWindow):
    def __init__(self):
        ...
        self.mainMessageTableWidget.itemChanged.connect(self.showBits)
        ...
        # new variables
        self.hq = []  #highlighted cells queue
        ...
        for i, currentQTableWidgetItem in enumerate(items):
            ...
            if (i > 0 and i < len(items) and len(hexs) > 0):
                ...
                for j in range(cc):
                    ...
                    if pi:
                       if  (pi.text() != item.text()): #if the data is different from the new
                        if self.highlightNewDataCheckBox.isChecked() and self.groupModeCheckBox.isChecked():
                            self.hq.append((i, j)) # store highlighted cell references
                            QTimer.singleShot(1000, self.hideBits) # start timer timeout for 1 second
                            item.setBackground(QColor(104, 37, 98))
                       else:
                           item.setBackground(pi.background()) #maintain the current colour
    ...
    def hideBits(self):
        for _ in range(len(self.hq)):  # get all highlighted cells in showBitsTableWidget
            i, j = self.hq.pop(0) # remove from list
            item = QTableWidgetItem() # get blank color
            self.showBitsTableWidget.item(i-1, j).setBackground(item.background()) #set blank color
