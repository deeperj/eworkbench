...
from PyQt5.QtCore import Qt, QCoreApplication, QTimer, pyqtSlot
...
class canSnifferGUI(QMainWindow, canSniffer_ui.Ui_MainWindow):
    def __init__(self):  
        ...
        self.setupUi(self)
        self.mainMessageTableWidget.itemClicked.connect(self.showBits) # wire-up
    ...
    def bin_string(self, hexstring):
        scale = 16 ## equals to hexadecimal

        num_of_bits = 8
        rv = ''
        try:
          rv = bin(int(hexstring, scale))[2:].zfill(num_of_bits)
          return rv
    ...
    def setVerticalheaders(self):
        ...
        self.showBitsTableWidget.setVerticalHeaderItem(0, item)

        item = QTableWidgetItem()
        item.setText(_translate("MainWindow", "RTR"))
        self.showBitsTableWidget.setVerticalHeaderItem(1, item)

        item = QTableWidgetItem()
        item.setText(_translate("MainWindow", "IDE"))
        self.showBitsTableWidget.setVerticalHeaderItem(2, item)

        item = QTableWidgetItem()
        item.setText(_translate("MainWindow", "DLC"))
        self.showBitsTableWidget.setVerticalHeaderItem(3, item)

        cc = self.showBitsTableWidget.rowCount()
        for i in range(4, cc):
            item = QTableWidgetItem()
            item.setText(_translate("MainWindow", f"DATA{(i-4)}"))
            self.showBitsTableWidget.setVerticalHeaderItem(i, item)

    def calc_bits(self):
        pc = self.showBitsTableWidget.rowCount()
        cc = self.showBitsTableWidget.columnCount()
        for i in range(pc):
            self.showBitsTableWidget.removeRow(0)
        items = self.mainMessageTableWidget.selectedItems()
        for i, currentQTableWidgetItem in enumerate(items):
            hexs = currentQTableWidgetItem.text().split()
            hexs = hexs[0] if len(hexs) > 0 else ''
            if (i>0 and i < len(items) and len(hexs) > 0):
                row = self.showBitsTableWidget.rowCount()
                self.showBitsTableWidget.insertRow(row)
                bstr = self.bin_string(hexs)
                #print(hexs)
                for j in range(cc):
                    if j == 0:
                        item = QTableWidgetItem(hexs)
                    elif j == 1:
                        item = QTableWidgetItem(str(int(hexs, 16)))
                    elif i > 4:
                        item = QTableWidgetItem(bstr[j-2])
                    else:
                        item = QTableWidgetItem('')
                    item.setTextAlignment(Qt.AlignVCenter | Qt.AlignHCenter)
                    self.showBitsTableWidget.setItem(row, j, item)
        self.setVerticalheaders()

    def showBits(self):
        pc = self.showBitsTableWidget.rowCount()
        cc = self.showBitsTableWidget.columnCount()
        items = self.mainMessageTableWidget.selectedItems() # get row from 
        for i, currentQTableWidgetItem in enumerate(items):
            hexs = currentQTableWidgetItem.text().split()
            hexs = hexs[0] if len(hexs) > 0 else ''
            if (i>0 and i < len(items) and len(hexs) > 0):
                ...
                for j in range(cc):
                    if j == 0:
                        ...
                        item = QTableWidgetItem('')
                    pi = self.showBitsTableWidget.item(i-1, j) # current item
                    if(pi and pi.text() != item.text()):
                        # if self.highlightNewDataCheckBox.isChecked() and self.groupModeCheckBox.isChecked():
                        self.hq.append((i, j))
                        item.setBackground(QColor(104, 37, 98))
                        self.timer.timeout.connect(self.highlightCellTimeout)
                        self.timer.start(1000)
                    item.setTextAlignment(Qt.AlignVCenter | Qt.AlignHCenter)
                    self.showBitsTableWidget.setItem(i-1, j, item)
            else:
                for j in range(cc):
                    item = QTableWidgetItem('')
                    item.setTextAlignment(Qt.AlignVCenter | Qt.AlignHCenter)
    ...
    def highlightCellTimeout(self):
        for k in range(len(self.hq)):
            i, j = self.hq.pop(0)
            self.showBitsTableWidget.item(i-1, j).setBackground(QColor(52, 44, 124))
